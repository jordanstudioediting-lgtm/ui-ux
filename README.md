...
| Code | Controller | Condition | Note |
|---|---|---|---|
| AC-01 | Banner Peringatan Stok | Jika | Salah satu varian gas memiliki persentase ketersediaan kurang dari 10% (level kritis) |
|  |  | Sewaktu | Halaman Dashboard dimuat |
|  |  | Maka | Banner peringatan dengan ikon segitiga oranye muncul di bagian atas halaman. Teks banner menyebutkan varian yang mencapai level kritis dan informasi bahwa sistem telah memberi notifikasi otomatis ke Agen mitra |
| AC-02 | Banner Peringatan Stok | Jika | Seluruh varian gas memiliki persentase ketersediaan di atas 10% |
|  |  | Sewaktu | Halaman Dashboard dimuat |
|  |  | Maka | Banner peringatan tidak ditampilkan |
| AC-03 | Kartu Statistik Stok | Jika | Halaman dimuat |
|  |  | Sewaktu | — |
...
| AC-04 | Tabel Transaksi Terkini | Jika | Tabel dimuat |
|  |  | Sewaktu | — |
...
| Deskripsi | Text | — | — | Conditional | No | Format: "Stok varian [Nama Varian] telah mencapai level kritis. Sistem GasTrack telah memberi notifikasi otomatis ke Agen mitra Anda." |  |
| **Kartu Statistik (4 kartu)** |  |  |  |  |  |  |  |
| Stok LPG 3kg | Integer | — | — | No | No | Contoh: 142 / 250 unit |  |
...
| Tipe Gas | Badge | — | — | No | No | Badge berwarna sesuai varian (LPG 3KG hijau, BRIGHT 5.5KG biru) |  |
...
Alur penggunaan dimulai dengan pemilik Pangkalan memilih nama pelanggan dari dropdown, kemudian memilih varian gas yang akan ditransaksikan (LPG 3 kg, Bright Gas 5.5 kg, atau LPG 12 kg). Setelah itu, pengguna memilih tipe transaksi (Isi Ulang, Pelanggan Beli Tabung, atau Pelanggan Jual Tabung). Apabila tipe transaksi yang dipilih adalah "Pelanggan Beli Tabung" atau "Pelanggan Jual Tabung", radio button **Kondisi Tabung** (Isi atau Kosong) akan muncul, sesuai catatan yang tertera pada antarmuka. Pengguna kemudian menentukan jumlah unit, dapat mengisi catatan tambahan, dan secara opsional dapat menggunakan fitur override harga untuk transaksi dengan negosiasi khusus. Tombol "Tambah" akan memasukkan item ke dalam keranjang.

Panel **Keranjang** di sebelah kanan menampilkan ringkasan transaksi termasuk total harga, serta menyediakan dua kartu pilihan metode pembayaran: **Tunai (Cash)** dan **Transfer Bank**. Berbeda dengan Manajemen Kiriman, halaman ini tidak mengharuskan unggah bukti transfer karena pelanggan tidak memiliki akun di sistem; metode pembayaran hanya digunakan sebagai pencatatan internal Pangkalan. Setelah memilih metode pembayaran, pengguna mengklik tombol "Simpan Transaksi" untuk finalisasi atau "Batalkan" untuk membatalkan.

Pada pojok kanan bawah halaman terdapat ikon **"default harga"** (gear) yang membuka drawer "Atur Harga Default" di sisi kanan. Drawer ini memungkinkan Pangkalan mengatur harga jual default untuk masing-masing varian dan kondisi tabung (Isi/Kosong), dengan harga dasar yang sebelumnya diberikan oleh Agen sebagai referensi. Harga default ini akan otomatis dipakai saat membuat transaksi baru, dan masih dapat di-override per transaksi jika ada negosiasi.
...
| AC-04 | Radio Kondisi Tabung | Jika | Pengguna memilih tipe transaksi "Pelanggan Beli Tabung" atau "Pelanggan Jual Tabung" |
|  |  | Sewaktu | Klik kartu tipe transaksi |
|  |  | Maka | Radio button Kondisi Tabung (Isi / Kosong) muncul dan wajib dipilih sebelum menekan tombol Tambah |
| AC-05 | Metode Pembayaran Keranjang | Jika | Pengguna belum memilih metode pembayaran (Tunai atau Transfer Bank) |
|  |  | Sewaktu | Klik tombol Simpan Transaksi |
|  |  | Maka | Muncul pesan error "Pilih metode pembayaran terlebih dahulu" |
| AC-06 | Tombol Simpan Transaksi | Jika | Keranjang berisi minimal satu item, pelanggan telah dipilih, dan metode pembayaran telah dipilih |
|  |  | Sewaktu | Klik tombol Simpan Transaksi |
|  |  | Maka | Transaksi disimpan, stok diperbarui, halaman direset, dan muncul notifikasi sukses |
| AC-07 | Tombol Plus Tambah Pelanggan | Jika | Pengguna mengklik tombol `+` di sebelah dropdown Nama Pelanggan |
|  |  | Sewaktu | Klik tombol |
|  |  | Maka | Drawer "Tambah Pelanggan Baru" muncul di sisi kanan halaman |
| AC-08 | Tombol Tambah Pelanggan (di drawer) | Jika | Nama pelanggan dan jenis pengguna telah terisi |
|  |  | Sewaktu | Klik tombol Tambah |
|  |  | Maka | Pelanggan baru disimpan, drawer ditutup, dan dropdown pelanggan otomatis terisi dengan nama baru |
| AC-09 | Ikon Default Harga | Jika | Pengguna mengklik ikon "default harga" pada pojok kanan bawah halaman |
|  |  | Sewaktu | Klik ikon |
|  |  | Maka | Drawer "Atur Harga Default" muncul di sisi kanan halaman |
| AC-10 | Tombol Simpan (Drawer Atur Harga) | Jika | Varian/Jenis Tabung, kondisi tabung (Isi/Kosong), dan Harga Default telah terisi |
|  |  | Sewaktu | Klik tombol Simpan |
|  |  | Maka | Harga default tersimpan dan otomatis dipakai pada transaksi baru yang sesuai varian dan kondisi tabung |
| AC-11 | Field Override Harga | Jika | Pengguna mengisi field Override Harga dengan nilai berbeda dari harga default |
|  |  | Sewaktu | Klik tombol Tambah |
...
| Pilihan Varian Gas | Radio Card | — | M | No | No | Opsi: LPG 3kg, Bright Gas 5.5kg, LPG 12kg. Default: LPG 3kg. Setiap kartu menampilkan stok terkini | Wajib dipilih |
...
| Pilihan Kondisi Tabung | Radio | — | Conditional | Conditional | No | Muncul ketika tipe transaksi = "Pelanggan Beli Tabung" atau "Pelanggan Jual Tabung". Opsi: Isi, Kosong | Wajib dipilih jika muncul |
...
| Metode Pembayaran | Radio Card | — | M | No | No | Dua kartu pilihan: Tunai (Cash) dan Transfer Bank. Tidak memerlukan upload bukti transfer karena pelanggan tidak memiliki akun di sistem | Wajib dipilih sebelum Simpan |
| Tombol Simpan Transaksi | Button | — | — | No | Conditional | Disabled jika keranjang kosong atau metode pembayaran belum dipilih |  |
| Tombol Batalkan | Button | — | — | No | No | Mengosongkan keranjang dan mereset form |  |
| **Drawer Tambah Pelanggan Baru** |  |  |  |  |  |  |  |
...
| Tombol Tambah (Drawer Pelanggan) | Button | — | — | No | Conditional | Disabled jika field wajib belum terisi |  |
| **Ikon Default Harga** |  |  |  |  |  |  |  |
| Ikon Default Harga | Icon Button | — | O | No | No | Ikon gear + label "default harga" di pojok kanan bawah, membuka drawer Atur Harga Default |  |
| **Drawer Atur Harga Default** |  |  |  |  |  |  |  |
| Varian/Jenis Tabung | Dropdown | — | M | No | No | Opsi: LPG 3kg, Bright Gas 5.5kg, LPG 12kg | Wajib dipilih |
| Kondisi Tabung | Radio | — | M | No | No | Opsi: Isi, Kosong | Wajib dipilih |
| Harga Default | Currency | 10 | M | No | No | Nilai harga default untuk kombinasi varian + kondisi tabung yang dipilih | Numerik > 0 |
| Tombol Simpan (Drawer Harga) | Button | — | — | No | Conditional | Disabled jika field wajib belum terisi |  |
...
| AC-02 | Filter Tipe Transaksi | Jika | Pengguna memilih opsi pada dropdown Tipe Transaksi |
|  |  | Sewaktu | Klik opsi |
|  |  | Maka | Tabel difilter menampilkan transaksi sesuai tipe yang dipilih |
...
| Filter Tipe Transaksi | Dropdown | — | O | No | No | Opsi: Semua Tipe Transaksi, Isi Ulang, Beli Tabung, Jual Tabung |  |
...
3. **Penerapan prinsip UI/UX.** Rancangan GasTrack menerapkan beberapa prinsip dasar UI/UX, antara lain Visibility of System Status (melalui badge status stok, badge status kiriman, banner peringatan kritis, dan progress bar persentase), Recognition over Recall (melalui ikon konsisten, kartu pilihan visual, dan pengkodean warna kategorial), dan Konsistensi (melalui sidebar identik, palet warna global, dan pola tata letak yang seragam di seluruh halaman).
...
2. **Tampilan responsive untuk perangkat mobile dan tablet.** Rancangan saat ini menampilkan UI/UX dalam orientasi desktop/PC untuk kemudahan visualisasi seluruh komponen. Pengembangan tampilan responsive untuk perangkat mobile dan tablet menjadi penting karena banyak pemilik Pangkalan yang lebih sering mengakses aplikasi melalui smartphone, dan ergonomis tampilan mobile akan jauh lebih praktis untuk operasi sehari-hari.

3. **Integrasi notifikasi push dan email.** Penambahan notifikasi otomatis ketika stok mencapai ambang batas kritis, ketika ada kiriman baru dari Agen, atau ketika pembayaran berhasil dikonfirmasi, akan meningkatkan responsivitas operasional Pangkalan. Notifikasi otomatis ini juga menjadi mekanisme yang dijanjikan pada banner peringatan stok kritis di Dashboard.

4. **Modul Laporan yang lebih kaya.** Pengembangan halaman Laporan Keuangan dapat mencakup analisis lebih dalam seperti perbandingan periode, prediksi stok berdasarkan tren historis, perhitungan margin keuntungan per varian, serta laporan khusus pelanggan terbanyak.

5. **Fitur Multi-User dalam Satu Pangkalan.** Pangkalan dengan skala operasional besar dapat memiliki lebih dari satu karyawan yang menggunakan aplikasi. Pengembangan fitur role-based access control (misalnya Pemilik, Kasir, Operator Stok) akan memungkinkan pembagian tugas dan jejak audit yang lebih jelas.

6. **Integrasi dengan MAP Pertamina dan BRIMOLA (opsional).** Apabila memungkinkan secara regulasi dan teknis, integrasi data antara GasTrack dengan MAP Pertamina (untuk data transaksi konsumen) dan BRIMOLA (untuk data kuota dan pembayaran) akan menghilangkan kebutuhan input ganda dan memberikan pengalaman yang lebih mulus bagi pemilik Pangkalan.
